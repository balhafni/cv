...
namespace PCA
{
    public class PCA
    {
        ...
        public Matrix GetMeanVector(List<Matrix> vectors)
        {
            ...
            {
                meanVector[i, 0] = meanVector[i, 0] / vectors.Count;
            }
            return meanVector;
        }
        ...
        public Matrix GetEigenVectorsMatrix(Matrix covarianceMatrix)
        {
            ...
            {
                eigenValues_Magnitudes[i] = Math.Abs(eigenValues[i]);
                lookup.Add(i, eigenValues_Magnitudes[i]);
            }

            //sort eigen values magnitude
            ...
            Matrix eigenVectors_Matrix = new Matrix(covarianceMatrix.Rows, 100);

            for (int i = 0; i < eigenVectors_Matrix.Rows; i++)
            {
                for (int j = 0; j < eigenVectors_Matrix.Columns; j++)
                {
                    eigenVectors_Matrix[i, j] = eigenVectors[lookup.FirstOrDefault(x => x.Value == eigenValues_Magnitudes[j]).Key, i];
                }
            }
        ...
        public int MatchImage(Bitmap unknownImage, List<Matrix> vectors, Matrix EigenFaces, List<Matrix> coefficients)
        {
            ...
            Matrix coefficients_unknown = new Matrix(1, 100);
